...
angular.module('docketrApp')
    .service('Customers', function ($resource) {
        ...
        self.add = function (customer, cbSuccess, cbError) {
            //console.log('add',customer);
            customer = angular.copy(customer);
            customer.car_plate_no = customer.carPlateNo;
            customer.tax_no = customer.taxNo;
            //delete customer['carPlateNo'];
            delete customer.carPlateNo;
            //delete customer['taxNo'];
            delete customer.taxNo;
            //Customers.add(customer, cbSuccess, cbError);
            var c = new Customers(customer);
            c.$save(cbSuccess, cbError);
        };
        
        // Update customer
        self.save = function (customer, cbSuccess, cbError) {
            customer = angular.copy(customer);
            customer.car_plate_no = customer.carPlateNo;
            customer.tax_no = customer.taxNo;
            //delete customer['carPlateNo'];
            ...
            //delete customer['taxNo'];
            delete customer.taxNo;
            Customers.update({ id: customer.id }, customer, cbSuccess, cbError);
        };

        // Remove customer
        self.remove = function (customer, cbSuccess, cbError) {
        ...
        self.refresh = function () {
            customers = Customers.query(function () {
                angular.forEach(customers, function (item, index) {
                    item.carPlateNo = item.car_plate_no;
                    item.taxNo = item.tax_no;
                });
            });
            return customers;
        };
  ...
  .service('Products', function ($resource) {
      ...
      self.save = function (products, cbSuccess, cbError) {
          products = angular.copy(products);

          // Remove amount field in this product collection.
          angular.forEach(products.products, function (item, index) {
              delete item.amount;
          });
          Products.save(products, cbSuccess, cbError);
      };
      ...
      self.refresh = function () {
          products = Products.query(function () {
